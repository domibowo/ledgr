...
function registerJournalHandlers() {
  ...
  ipcMain.handle("journal:export-excel", async (_e, companyId, filters) => {
    let sql = `
      SELECT je.*,
        (SELECT SUM(amount) FROM journal_lines WHERE entry_id = je.id AND type = 'DEBIT')  AS total_debit,
        (SELECT SUM(amount) FROM journal_lines WHERE entry_id = je.id AND type = 'CREDIT') AS total_credit
    ...
    `).all(companyId);
    const entrySheet = utils.json_to_sheet(entries.map((e) => ({
    ...
    const lineSheet = utils.json_to_sheet(lines.map((l) => ({
    ...
    utils.book_append_sheet(wb, entrySheet, "Jurnal");
    utils.book_append_sheet(wb, lineSheet, "Baris Jurnal");
    const { filePath, canceled } = await dialog.showSaveDialog({
      title: "Ekspor Jurnal ke Excel",
      defaultPath: `jurnal-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.xlsx`,
